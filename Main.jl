...
function optimize_edisgo()                                        
  ...
  if method == "soc" # Second order cone
    ...
    gurobi = optimizer_with_attributes(Gurobi.Optimizer, MOI.Silent() => silence_moi, "Presolve" => 1)#, "BarQCPConvTol" => 1e-4, "BarConvTol" => 1e-6, "BarHomogeneous"=> 1)#,"FeasibilityTol"=>1e-4, "NumericFocus"=> 1)
    # Solve SOC model
