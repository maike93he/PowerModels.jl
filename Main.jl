...
function optimize_edisgo()                                        
  ...
  if method == "soc" # Second order cone
    ...
    gurobi = optimizer_with_attributes(Gurobi.Optimizer, "LogToConsole" => 0, MOI.Silent() => silence_moi, "Presolve" => 1, "FeasibilityTol"=>1e-4, "QCPDual" =>1, "BarQCPConvTol" => 1e-12, "BarHomogeneous"=> 1, "NumericFocus"=> 1)
    # Solve SOC model
