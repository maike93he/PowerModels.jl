...
function constraint_power_balance(pm::AbstractBFModelEdisgo, n::Int, i, bus_gens, bus_gens_nd, bus_gens_slack, bus_loads, bus_arcs_to, bus_arcs_from, bus_lines_to, bus_storage, bus_pg, bus_qg, bus_pg_nd, bus_qg_nd, bus_pd, bus_qd, branch_r, branch_x, bus_dsm, bus_hps, bus_cps, bus_storage_pf, bus_dsm_pf, bus_hps_pf, bus_cps_pf, bus_gens_pf, bus_loads_pf)
    ...
    #ps   = get(var(pm, n),  :ps, Dict()); _check_var_keys(ps, bus_storage, "active power", "storage") # die Aktivieren wenn eff = 1
    sc   = get(var(pm, n),  :sc, Dict()); _check_var_keys(sc, bus_storage, "charge", "storage") 
    sd   = get(var(pm, n),  :sd, Dict()); _check_var_keys(sd, bus_storage, "charge", "storage") 
    ...
    if ref(pm, 1, :opt_version) in(2, 4)
        ...
        cstr_p = JuMP.@constraint(pm.model,
            ...
            + sum(sc[s] - sd[s] for s in bus_storage)
        ...
        cstr_q = JuMP.@constraint(pm.model,
            ...
            + sum((sc[s] - sd[s]) * bus_storage_pf[s] for s in bus_storage)
